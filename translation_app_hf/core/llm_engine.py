...
class LLMEngine:
    ...
    def translate(self, text: str, source_lang: str, target_lang: str, temperature=0.3, top_p=0.9, top_k=40, rag_context: str = None):
        ...
        if self.is_gemini:
            ...
            # If RAG context is provided (full prompt string from RAG engine), use it instead or augment
            if rag_context:
                # When RAG context is present, it usually contains the full instruction + examples + text
                # So we can just use the provided rag_context as the user message, or prepend system instructions.
                # The format_rag_prompt_from_rag_engine returns a block with examples and the text.
                # We will wrap it with a specific instruction if needed, but the RAG prompt already has instructions.
                # Let's trust the RAG prompt but add the role.
